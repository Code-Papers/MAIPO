...
class CategoricalPd(Pd):
    ...
    def mode(self):
        return U.argmax(self.logits, axis=1)
    def logp(self, x):
        if x.dtype in {tf.uint8, tf.int32, tf.int64, tf.float32}:
    ...
    #     z0 = U.sum(ea0, axis=1, keepdims=True)
    #     p0 = ea0 / z0
    #     dis = tf.squre(p0 - 1.0 / self.logits.get_shape().as_list()[-1])
    #     return U.sum(dis, axis=1)
...
class SoftCategoricalPd(Pd):
    ...
    def kl(self, other):
        a0 = self.logits - U.max(self.logits, axis=1, keepdims=True)
        a1 = other - U.max(other, axis=1, keepdims=True)
        ea0 = tf.exp(a0)
        ea1 = tf.exp(a1)
        z0 = U.sum(ea0, axis=1, keepdims=True)
        z1 = U.sum(ea1, axis=1, keepdims=True)
    ...
    #     p0 = ea0 / z0
    #     p1 = ea1 / z1
    #     return U.sum(tf.abs(p0-p1), axis=1)
    def entropy(self):
        a0 = self.logits - U.max(self.logits, axis=1, keepdims=True)
        ea0 = tf.exp(a0)
        z0 = U.sum(ea0, axis=1, keepdims=True)
    ...
    #     z0 = U.sum(ea0, axis=1, keepdims=True)
    #     p0 = ea0 / z0
    #     dis = tf.square(p0 - 1.0 / self.logits.get_shape().as_list()[-1])
    #     return U.sum(dis, axis=1)
    def sample(self):
        u = tf.random_uniform(tf.shape(self.logits))
        output = U.argmax(U.softmax(self.logits - tf.log(-tf.log(u)), axis=-1), axis=-1)
        output = tf.expand_dims(output, axis=-1)
        output = tf.cast(output, dtype=tf.float32)
        # output = tf.one_hot(num, self.logits.get_shape().as_list()[-1])
        return output
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)        
...
class SoftMultiCategoricalPd(Pd):  # doesn't work yet
    ...
    def logp(self, x):
        return tf.add_n([p.logp(px) for p, px in zip(self.categoricals, tf.split(x, self.ncat, axis=len(x.get_shape()) - 1))])
    def kl(self, other):
        return tf.add_n([
                p.kl(q) for p, q in zip(self.categoricals, other.categoricals)
            ])
    ...
    def sample(self):
        x = []
        for i in range(len(self.categoricals)):
            x.append(self.categoricals[i].sample())
        return tf.concat(x, axis=-1)
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
