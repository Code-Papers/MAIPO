...
from trainer.happo import HAPPOAgentTrainer, HAPPOAgentsTrainer
...
def parse_args():
    ...
    parser.add_argument("--lr", type=float, default=7e-4, help="learning rate for Adam optimizer")
    parser.add_argument("--gamma", type=float, default=0.99, help="discount factor")
    parser.add_argument("--lam", type=float, default=0.95, help="lambda return")
    parser.add_argument("--clip-range", type=float, default=0.2, help="clip range in MAPO")
    ...
    parser.add_argument("--plots-dir", type=str, default="./learning_curves4/", help="directory where plot data is saved")
...
def get_trainers(env, num_agents, num_adversaries, obs_shape_n, action_space, arglist):
    ...
    for i in range(num_adversaries):
        trainers.append(trainer(
            "agent_%d" % i, model, obs_shape_n, action_space, i, clip_range, arglist))
    for i in range(num_adversaries, num_agents):
        trainers.append(trainer(
            "agent_%d" % i, model, obs_shape_n, action_space, i, clip_range, arglist))
    group_trainer = MAPOAgentsTrainer("group_trainer", model, obs_shape_n, arglist.clip_range, arglist)
...
def train(arglist):
    with U.single_threaded_session():
        ...
        trainers, group_trainer = get_trainers(env, num_agents, num_adversaries, obs_shape_n,  action_space, arglist)
        ...
        while True: 
            # collect experience
            samples, episode_rewards, agent_rewards = runner.run(trainers, group_trainer)
            weight = 0
            agents_obs = []
            group_ret     = samples[2]
            group_adv     = samples[4]
            ...
            for i, agent in enumerate(trainers):
                ...
                agent_samples = [agent_obs_t, agent_act, agent_logp]
                agent.experience(agent_samples)
            v_loss = group_trainer.update(arglist.batch_size, agents_obs, group_val, group_ret)
            M_adv = group_adv 
