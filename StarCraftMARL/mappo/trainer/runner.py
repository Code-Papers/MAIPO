...
class Runner(object):
    ...
    def run(self, trainers):
        mb_state, mb_value, mb_ava_actions, mb_action, mb_logp, mb_reward, mb_state1, mb_done, env_done, mb_info = [], [], [], [], [], [], [], [], [], []
        obs_n = self.obs0
        num_agents = len(trainers)
        for _ in range(self.nsteps):
            action_n, ava_actions, logp_n, value_n = [], [], [], []
            for i, agent in enumerate(trainers):
                avail_actions = self.env.get_avail_agent_actions(i)
                # print(i)
                # print(np.shape(obs_n[i]))
                # print(obs_n[i])
                agent_act, agent_logp, agent_val = agent.step(obs_n[i], avail_actions)
        ...
        for i, agent in zip(range(num_agents), trainers):
            if done_n[i]:
                ...
                last_values.append(agent.value_s([obs_n[i]])[0])
        ...
        samples = [mb_state, mb_action, mb_return, mb_logp, mb_adv, mb_value, mb_ava_actions]
